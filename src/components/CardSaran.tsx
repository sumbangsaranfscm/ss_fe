  ...
  return (
    ...
    >
      <div className="col-span-2 p-2">
        {cardKomite ? (
          <div
            className={`flex h-full w-full items-center justify-center rounded-lg border ${_.status_komite.trim().length === 0 ? "bg-blue-500" : "bg-neutral-400"}`}
          >
            <FileText size={30} className="text-neutral-50" />
          </div>
        ) : (
          <div
            className={`flex h-full w-full items-center justify-center rounded-lg border ${_.status_a.trim().length === 0 ? "bg-blue-500" : "bg-neutral-400"}`}
          >
            <FileText size={30} className="text-neutral-50" />
          </div>
        )}
